...
/**
 * Représente la polaire d'un bateau, L'objet est de retourner une vitesse de
 * bateau selon une vitesse de vent et un angle de bateau par rapport au vent.
 * Si la polaire est incomplète elle doit être interpolée.
 *
 * L'interpolation est faite à la demande.
 *
 * On utilise des TreeMap pour stocker les valeurs, ainsi on peut récupérer les
 * valeurs immédiatement inférieur et supérieure par rapport à une clé.
 * 
 * @author Fred
 *
 */
public class Polaire {
    ...
    private float calculVitesseBateau(TreeMap<Float, BaWsBs> arbreVitesse, Float vitesseVent) {
        ...
        if (boatSpeed == null) {
            ...
            retourVitesse = (highEntry.getValue().boatSpeed - floorEntry.getValue().boatSpeed) / ratioVitesse
                    + floorEntry.getValue().boatSpeed;
    ...
    public void addAngle(float angle, float[] vitesseVent, float[] vitesseBateau) {
        TreeMap<Float, BaWsBs> arbreVitesse = new TreeMap<Float, Polaire.BaWsBs>();
        for (int i = 0; i < vitesseVent.length; i++) {
            arbreVitesse.put(Float.valueOf(vitesseVent[i]), new BaWsBs(vitesseVent[i], angle, vitesseBateau[i]));
        }
